...
{
	public interface IDatabaseInitializer { Task SeedAsync(); }
	...
	public class DatabaseInitializer : IDatabaseInitializer
	{
		...
		public async Task SeedAsync()
		{
			...
			if (await DbContext.Shops.CountAsync() == 0) CreateShops();
		}
		...
		private async Task CreateUsersAsync()
		{
			...
			if (!await RoleManager.RoleExistsAsync(roleAdministrators)) await RoleManager.CreateAsync(new IdentityRole(roleAdministrators));
			if (!await RoleManager.RoleExistsAsync(roleRegistered)) await RoleManager.CreateAsync(new IdentityRole(roleRegistered));
			...
			if (await UserManager.FindByIdAsync(user_Ryan.Id) == null)
			{
				...
				await UserManager.AddToRoleAsync(user_Ryan, roleRegistered);
				// Remove Lockout and E-Mail confirmation.
			...
			if (await UserManager.FindByIdAsync(user_Solice.Id) == null)
			{
				...
				await UserManager.AddToRoleAsync(user_Solice, roleRegistered);
				// Remove Lockout and E-Mail confirmation.
			...
			if (await UserManager.FindByIdAsync(user_Vodan.Id) == null)
			{
				...
				await UserManager.AddToRoleAsync(user_Vodan, roleRegistered);
				// Remove Lockout and E-Mail confirmation.
		...
		private void CreateShops()
		{
			...
			{
				DbContext.Shops.Add(GetSampleShop(id, authorId, num - id, new DateTime(2015, 12, 31).AddDays(-num)));
			}

#endif

			EntityEntry<Shop> e1 = DbContext.Shops.Add(new Shop()
			{
				UserId = authorId,
				Title = "Magarena",
				Description = "Single-player fantasy card game similar to Magic: The Gathering",
				Text = @"Loosely based on Magic: The Gathering,
                the game lets you play against a computer opponent or another human being.
                The game features a well-developed AI,
                an intuitive and clear interface and an enticing level of gameplay.",
			...
			EntityEntry<Shop> e2 = DbContext.Shops.Add(new Shop()
			{
				UserId = authorId,
				Title = "Minetest",
				Description = "Open-Source alternative to Minecraft",
				Text = @"The Minetest gameplay is very similar to Minecraft's:
                you are playing in a 3D open world,
                where you can create and/or remove various types of blocks.
                Minetest feature both single-player and multiplayer game modes.
                It also has support for custom mods,
                additional texture packs and other custom/personalization options.
			...
			EntityEntry<Shop> e3 = DbContext.Shops.Add(new Shop()
			{
				UserId = authorId,
				Title = "Relic Hunters Zero",
				Description = "A free game about shooting evil space ducks with tiny, cute guns.",
				Text = @"Relic Hunters Zero is fast,
                tactical and also very smooth to play.
                It also enables the users to look at the source code,
                so they can can get creative and keep this game alive,
			...
			EntityEntry<Shop> e4 = DbContext.Shops.Add(new Shop()
			{
				UserId = authorId,
				Title = "SuperTux",
				Description = "A classic 2D jump and run, side-scrolling game similar to the Super Mario series.",
				Text = @"The game is currently under Milestone 3.
                The Milestone 2, which is currently out, features the following:
			...
			EntityEntry<Shop> e5 = DbContext.Shops.Add(new Shop()
			{
				UserId = authorId,
				Title = "Scrabble3D",
				Description = "A 3D-based revamp to the classic Scrabble game.",
				Text = @"Scrabble3D extends the gameplay of the classic game Scrabble by adding a new whole third dimension.Other than playing left to right or top to bottom,
                you'll be able to place your tiles above or beyond other tiles.
                Since the game features more fields,
                it also uses a larger letter set.
                You can either play against the computer,
                players from your LAN or from the Internet.
                The game also features a set of game servers where you can challenge players from all over the world and get ranked into an official,
                ELO-based rating/ladder system. ",
				Notes = "This is a sample record created by the Code-First Configuration class",
				ViewCount = 6754,
				CreatedDate = createdDate,
				LastModifiedDate = lastModifiedDate
		...
		/// <param name="createdDate">the item CreatedDate</param>/// <returns></returns>
		private Shop GetSampleShop(int id, string authorId, int viewCount, DateTime createdDate)
		{
			return new Shop()
			{
				UserId = authorId,
